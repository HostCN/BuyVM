...
TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID', '@zxtspd')
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
            ...
            if not content:
                logger.warning(f"{CONFIG_FILE} 为空，使用默认空配置")
                return {}
            try:
                config = json.loads(content).get("products", {})
                logger.debug(f"成功加载配置: {config}")  # 保持 DEBUG
                return config
            except json.JSONDecodeError as e:
                logger.error(f"解析 {CONFIG_FILE} 失败: {e}")
                return {}
    logger.error(f"{CONFIG_FILE} 不存在，请创建配置文件")
    return {}
...
def save_product_info(product_info):
    try:
        ...
        logger.debug("商品信息已保存")  # 调整为 DEBUG
        ...
        while retries < MAX_RETRIES:
            try:
                if await is_message_already_sent(message):
                    logger.info("消息已存在，跳过发送")  # 保持 INFO
                    return None
                ...
                logger.info(f"消息发送成功: {message[:30]}...")  # 缩短日志
                await asyncio.sleep(random.uniform(1, 3))
                return sent_message
            except RetryAfter as e:
                logger.warning(f"达到速率限制，等待 {e.retry_after} 秒")
                await asyncio.sleep(e.retry_after)
            except TimedOut:
                retries += 1
                logger.warning(f"发送超时，重试 {retries}/{MAX_RETRIES}")
...
async def edit_telegram_message(chat_id, message_id, new_message):
    ...
    while retries < MAX_RETRIES:
        try:
            ...
            logger.info(f"编辑消息成功: {new_message[:30]}...")  # 缩短日志
            await asyncio.sleep(random.uniform(1, 3))
            return True
        except RetryAfter as e:
            logger.warning(f"达到速率限制，等待 {e.retry_after} 秒")
            ...
            retries += 1
            logger.warning(f"编辑超时，重试 {retries}/{MAX_RETRIES}")
            await asyncio.sleep(2)
        except telegram.error.BadRequest as e:
            if "Message is not modified" in str(e):
                logger.info("消息未更改，跳过编辑")  # 保持 INFO
                return True
            elif "Message to edit not found" in str(e):
                logger.warning("消息未找到，可能已被删除")
                return False
...
def build_product_message(name, price, features_str, availability, full_link, remark=""):
    ...
    logger.debug(f"构建消息 - 商品: {name}, remark: {remark}")  # 保持 DEBUG
    return (
...
async def fetch_and_parse_products(url, product_info, config, first_run=False):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=TIMEOUT, ssl=ssl_context) as response:
                    if response.status != 200:
                        logger.warning(f"网页状态码 {response.status}，跳过: {url}")
                        return
                    html = await response.text()
                    soup = BeautifulSoup(html, 'html.parser')
                    title_tag = soup.find('title')
                    if title_tag and "FranTech" not in title_tag.get_text(strip=True):
                        logger.warning(f"标题不含 'FranTech'，跳过: {url}")
                        return
                    break
        except Exception as e:
            retries += 1
            logger.warning(f"请求失败，重试 {retries}/{MAX_RETRIES}")
            await asyncio.sleep(2)
    ...
    logger.info(f"找到 {len(products)} 个商品: {url}")  # 保持 INFO
    ...
    for product in products:
        try:
            ...
            if not link:
                logger.warning(f"商品 {name} 无购买链接，跳过")
                continue

            if name not in config:
                logger.info(f"商品 {name} 不在配置中，跳过")  # 保持 INFO
                continue
            ...
            logger.debug(f"商品 {name} - notify: {notify}, remark: {remark}")  # 保持 DEBUG

            if notify is None:
                logger.warning(f"商品 {name} 的 notify 未定义，跳过")
                continue
            ...
            if notify:
                if is_inventory_changed(previous_qty, qty, previous_status, current_status):
                    if current_status == 'in_stock':
                        ...
                        if name in product_info and product_info[name].get('message_id'):
                            ...
                            if edit_success:
                                logger.info(f"库存变为0，消息已编辑: {name}")  # 保持 INFO
                        product_info[name] = {
                            'qty': qty,
                ...
                logger.debug(f"商品 {name} 的 notify 为 False，跳过消息发送")  # 保持 DEBUG
                product_info[name] = {
                    'qty': qty,
            ...
            logger.error(f"解析商品 {name} 时出错: {e}")
